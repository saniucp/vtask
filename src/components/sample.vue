    ...
    async sortUsers() {
      try {
        ...
        if (this.sortOrder === 'APPROVED') {
          const response = await axios.get('http://192.168.11.213:3000/registration/get-all-user?order=1');
          this.users = response.data.getall;
        } else if (this.sortOrder === 'DISAPPROVED') {
          const response = await axios.get('http://192.168.11.213:3000/registration/get-all-user?order=0');
          this.users = response.data.getall;
        } else {
